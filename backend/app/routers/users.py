...
async def me(user: dict = Depends(get_current_user)):
    ...
    profile = r.data or user

    try:
        from app.services.gamification_service import get_level, compute_badges
        count = db.table("capsules").select("id", count="exact").eq("user_id", user["sub"]).execute()
        total = count.count or 0
        profile["level"] = get_level(total)
        profile["badges"] = await compute_badges(user["sub"])
        profile["total_capsules"] = total
    except Exception:
        pass
